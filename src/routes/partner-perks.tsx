      ...
      <div className="max-w-3xl w-full space-y-12">
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <PerkCard
              ...
              description="On-call transportation to anywhere you need to go, I'm not letting another girl touch a car so please just sit and relax."
            />
            <PerkCard
              icon={<Zap />}
              title="Electrician"
              description="Fixing lights, switches, and anything that sparks."
            />
            <PerkCard
              icon={<Wrench />}
              title="Plumber"
              description="Handling leaks and clogs so you don't have to."
            />
            <PerkCard
              icon={<Wifi />}
              title="Internet Guy"
              description="Guaranteed 99.9% uptime and tech support. 24/7"
            />
            <PerkCard
              icon={<Monitor />}
              title="Computer Tech"
              description="Hardware repairs, software fixes, and custom builds (hardware is expensive, bring your own)."
            />
            <PerkCard
              icon={<Camera />}
              title="Photographer"
              description="Professional photo shoots for your Instagram."
            />
          </div>
        </section>
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <PerkCard
              icon={<HardDrive />}
              title="36TB NAS Access"
              description="Unlimited storage, for files and photo backups."
            />
            <PerkCard
              icon={<Film />}
              title="Personal Media Collection"
              description="200+ Anime, 60+ Movies, growing daily."
            />
            <PerkCard
              icon={<Youtube />}
              title="YouTube Premium"
              description="there's one free slot."
            />
            <PerkCard
              icon={<Server />}
              title="Web Hosting & Compute"
              description="Free resources for your silly projects."
            />
            <PerkCard
              icon={<Cloud />}
              title="iCloud+ Subscription"
              description="if you're an apple sheep, welcome."
            />
            <PerkCard
              icon={<Mail />}
              title="Custom Email"
              description="Your own @ckl.moe email address."
            />
            <PerkCard
              icon={<MapPin />}
              title="Real-time Location"
              description="other guys hesitant when giving you this, I'm not going anywhere anyway. (iSheep only)"
            />
            <PerkCard
              icon={<Cpu />}
              title="Hardware Lab Access"
              description="Full access to my lab and extensive microcontroller collection."
            />
          </div>
        </section>
            ...
            <div className="bg-red-50 dark:bg-red-900/20 p-6 rounded-2xl border border-red-100 dark:border-red-900/30 flex gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-red-600 dark:text-red-300 leading-relaxed">
                  For the love of god, please have a clear plan when going
                  anywhere. Don't change the plan every second. My GPS runs on
                  logic, not vibes.
                </p>
              </div>
            </div>
        ...
        <div className="p-8 bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-pink-100 dark:border-pink-900/20 text-center space-y-4 transform hover:scale-[1.02] transition-transform">
          ...
          <p className="text-slate-600 dark:text-slate-400 italic">
            "My mom said having me around make her life much easier, you get all
            that and more."
          </p>
        </div>
